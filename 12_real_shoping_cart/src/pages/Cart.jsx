...
const Cart = () => {
  ...
  //(or above line is equivalentto) => const cart = useSelector((state) => state.cart);
  const [totalAmount, setTotalAmount] = useState(0)

  useEffect(()=>{
    setTotalAmount(cart.reduce((acc,curr) => acc+curr.price, 0))    //add total sum of a cart price by using 'reduce' function
  }, [cart])                                                        //'setTotalAmount' always be call whenever any changes happen in 'cart'
